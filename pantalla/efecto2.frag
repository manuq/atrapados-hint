// Desde: https://www.shadertoy.com/view/ldXGW4
...
void main() {
	...
	float red 	=   texture(	uSampler, 	vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y) * 0.5).r+staticVal;
	float green = 	texture(	uSampler, 	vec2(uv.x + xOffset,	  y)* 0.5).g+staticVal;
	float blue 	=	texture(	uSampler, 	vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)* 0.5).b+staticVal;
