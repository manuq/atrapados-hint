// Desde: https://www.shadertoy.com/view/WsBGRW
...
vec3 color(sampler2D tex, vec2 uv){        
    vec3 color = texture(uSampler,uv*0.5).rgb;
    #ifdef COLOR
...
vec3 ghost(sampler2D tex, vec2 uv){
    ...
    float r = color(uSampler,uv + or).r; // FIXME uv*0.5?
    float g = color(uSampler + og).g;// FIXME uv*0.5?
    float b = color(uSampler,uv + ob).b;// FIXME uv*0.5?
    vec3 color = vec3(r,g,b);
    return color;
    #else 
    return texture(uSampler,uv*0.5).rgb;
    #endif
}
...
void main() {
    ...
    // if(iMouse.z>0.){
    //     fragColor = texture(uSampler,uv*0.5); 
    //     return;
    // }
    ...
    #ifdef BLUR
    vec3 colorA = ghost(uSampler,uv + onePixel,or,og,ob); // FIXME uv*0.5?
    vec3 colorB = ghost(uSampler,uv - onePixel,or,og,ob); // FIXME uv*0.5?
    vec3 colorC = ghost(uSampler,uv,or,og,ob); // FIXME uv*0.5?
    vec3 color = (colorA+colorB+colorC)/3.0;
    #else
    vec3 color = ghost(uSampler,uv); // FIXME uv*0.5?
    #endif
